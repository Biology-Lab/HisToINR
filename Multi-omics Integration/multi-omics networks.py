...
class Fusion(nn.Module):
    ...
    def __init__(self):
        ...
        self.image_proj = nn.Linear(1024, 128)
        self.alpha = nn.Parameter(torch.tensor(0.3))


    def forward(self, z_st, z_img):

        z_img = self.image_proj(z_img)

        z = z_st + self.alpha * z_img
...
class DeconvNet(nn.Module):
    ...
    def __init__(self,
        ...
        # maker gene embedding
        self.marker_gene_mlp = nn.Sequential(SineLayer(7, mid_channel),
                                             SineLayer(mid_channel, mid_channel),
                                             SineLayer(mid_channel, 30),
                                             DenseLayer(30, hidden_dims[0])
                                             )
    ...
    def forward(self,
        ...
        self.image_feat_uni = image_feat_uni

        # marker gene table
        self.m = m

        Z, mid_fea = self.encoder(node_feats)
        Z = self.fusion(Z,self.image_feat_uni)
        ...
        # Clamp cell count to avoid division by zero
        eps = 1e-6
        cell_count_clamp = torch.clamp(cell_count, min=eps)
        # Compute weighted predicted RNA count per spot, normalized by cell count
        weighted_pred = torch.sum(count_matrix * lam, dim=1) / cell_count_clamp
        # Compute actual RNA count per spot normalized by cell count
        weighted_actual = library_size / cell_count_clamp
        # # Compute cell loss (spots with more cells are given higher weight)
        cell_loss = torch.mean(weighted_pred - weighted_actual)

        # Total loss
        loss = 1 * (self.decon_loss + self.fea_loss) + 0.1 * cell_loss
    ...
    def encoder(self, H):
        ...
        maker_gene_emb = self.marker_gene_mlp(self.m) # extend: marker gene embedding feature
        self.mid_fea = self.mid_fea + maker_gene_emb
